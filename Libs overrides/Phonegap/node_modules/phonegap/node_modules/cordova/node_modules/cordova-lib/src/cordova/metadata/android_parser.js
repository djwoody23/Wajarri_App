...
android_parser.prototype.handleSplashes = function(config) {
    ...
    if (resources.length > 0) {
        this.deleteDefaultResource(filename + '.png');
        events.emit('verbose', 'splash screens: ' + JSON.stringify(resources));

        var projectRoot = util.isCordova(this.path);

        if (resources.defaultResource) {
            me.copyImage(path.join(projectRoot, resources.defaultResource.src), '', filename + '.png');
        }
        resources.forEach(function (resource) {
            if (!resource.density) {
                return;
            }
            me.copyImage(path.join(projectRoot, resource.src), resource.density, filename + '.png');
        });
    }
};
